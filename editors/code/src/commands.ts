...
function traitGraph(ctx: CtxInit, full: boolean): Cmd {
    return async () => {
        const nodeModulesPath = vscode.Uri.file(path.join(ctx.extensionPath, "node_modules"));

        const panel = vscode.window.createWebviewPanel(
            "rust-analyzer.trait-graph",
            "rust-analyzer trait graph",
            vscode.ViewColumn.Two,
            {
                enableScripts: true,
        ...
        const client = ctx.client;
        const dot = await client.sendRequest(ra.viewTraitGraph, params);
        const uri = panel.webview.asWebviewUri(nodeModulesPath);
...
export function viewTraitGraph(ctx: CtxInit): Cmd {
    return traitGraph(ctx, false);
}
